"""Agentless context gatherer augmented with BM25 retrieval.

Identical to the standard Agentless gatherer but injects BM25 keyword search
results into Phase 1 (file-level localization) so the LLM has ranked
candidates alongside the full file listing.
"""
...
class AgentlessBM25Gatherer(ContextGatherer):
    """Agentless 3-phase approach augmented with BM25 for file localization."""
    ...
    def gather(self, instance: BenchmarkInstance) -> GatherResult:
        ...
        # Re-use cached BM25 index if available
        ...
        # ── BM25 pre-search ──────────────────────────────────────
        bm25_results = bm25_index.search(instance.query, top_k=self.bm25_top_k)
        bm25_lines = []
        bm25_paths = []
        for rank, (path, score) in enumerate(bm25_results, 1):
            bm25_lines.append(f"  {rank}. {path}  (score: {score:.4f})")
            bm25_paths.append(path)
        bm25_output = "\n".join(bm25_lines) if bm25_lines else "No results found."

        trace.append(
            {
                "phase": "bm25_presearch",
                "results": [
                    {"file": p, "score": round(s, 4)} for p, s in bm25_results
                ],
            }
        )

        # ── Phase 1: File-level localization (with BM25 hints) ───
        ...
        phase1_msgs = [
            ...
            {
                "role": "user",
                "content": AGENTLESS_BM25_FILE_LOCALIZATION_PROMPT.format(
        ...
        # Fall back to BM25 results if LLM didn't return useful results
        if not candidate_files:
            candidate_files = list(bm25_paths)
        ...
        file_contents = ""
        for fpath in candidate_files[:5]:
    ...
    # ------------------------------------------------------------------
    # Helpers (shared with AgentlessGatherer)
    # ------------------------------------------------------------------
    ...
    def _build_targeted_regions(
        ...
        for region in regions:
            ...
            try:
                ...
                e = min(total, end)
                numbered = [
                    f"{s+1+i:>4}: {line}" for i, line in enumerate(all_lines[s:e])
                ]
                content = "\n".join(numbered)
                if e < total:
                    content += f"\n... ({total - e} more lines)"
